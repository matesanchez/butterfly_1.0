...
def search(query: str, max_results: int = 100) -> List[str]:
    r = retry_request(f'{BASE}esearch.fcgi', _params({'db': 'pubmed', 'term': query, 'retmax': max_results}))
    if not r:
        return []
    return r.json().get('esearchresult', {}).get('idlist', [])

def fetch_summary(pmid: str) -> dict:
    r = retry_request(f'{BASE}esummary.fcgi', _params({'db': 'pubmed', 'id': pmid}))
    if not r:
        return {}
    return r.json().get('result', {}).get(pmid, {})

def fetch_abstract(pmid: str) -> Optional[str]:
    r = retry_request(urljoin(BASE, 'efetch.fcgi'), _params({'db': 'pubmed', 'id': pmid, 'rettype': 'abstract', 'retmode': 'text'}))
    if not r:
        return None
    text = r.text.strip()
    if not text:
